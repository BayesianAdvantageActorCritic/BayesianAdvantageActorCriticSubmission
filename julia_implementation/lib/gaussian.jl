...
function EMA(g_old::Gaussian1d, g_new::Gaussian1d, β::FloatType, taus_observation::Vector{FloatType}, precision_observation::Vector{FloatType})
    # println("g_old.τ: $(g_old.τ), g_old.ρ: $(g_old.ρ), g_new.τ: $(g_new.τ), g_new.ρ: $(g_new.ρ)")
    τ = (1 - β) * g_old.τ + β * g_new.τ
    ρ = (1 - β) * g_old.ρ + β * g_new.ρ

    # # Create an EMA Gaussian with 0.999 for current values of τ and ρ and 0.001 for just default 1.0
    # τ = 0.999 * τ + 0.001 * 1.0
    # ρ = 0.999 * ρ + 0.001 * 1.0

    # Clamp the parameters between -1000 and 1000
    τ = clamp(τ, -10000, 10000)
    ρ = clamp(ρ, 1e-4, 10000)

    push!(taus_observation, τ)
    push!(precision_observation, ρ)
