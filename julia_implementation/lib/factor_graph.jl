...
function store_likelihood_messages(node::WeightFactor, out_W::AbstractArray{Gaussian1d}, out_bias::AbstractVector{Gaussian1d}; β_EMA::FloatType=1.0, taus_observation::Vector{FloatType}, precisions_observation::Vector{FloatType})
    ...
    out_W .= EMA.(out_W, reshape(out_W_new, size(out_W_new)[1:(end-1)]), β_EMA, [taus_observation], [precisions_observation])
...
function recompute_marginal(node::WeightFactor)
    ...
    # println("Before recompute, last_W_marginal was $(node.last_W_marginal)")
    prod!(reshape(node.last_W_marginal, size(node.last_W_marginal)..., 1), node.m_to_weights)
    # println("After recompute, last_W_marginal is $(node.last_W_marginal)")
...
function reset_for_batch(node::WeightFactor, new_prior::AbstractArray{Gaussian1d}, new_bias_prior::AbstractVector{Gaussian1d})
    ...
    # println("In reset_for_batch, last_W_marginal is $(node.last_W_marginal)")
...
function backward_message_adaptive_std(node::RegressionFactor, y::AbstractVector{FloatType}, std::AbstractVector{FloatType}, i_in::Int)
    β2 = node.β2
    @tullio node.m_back[i] = Gaussian1d(; μ=y[i], σ2=β2+std[i]^2)
    return node.m_back
end
...
function backward_pass(fg::FactorGraph, y::Union{Int,AbstractVector{FloatType}}, stds::AbstractVector{FloatType}, i_in::Int)
    m_back = backward_message_adaptive_std(fg.layers[end], y, stds, i_in)
...
function train_batch(fg::FactorGraph, X::AbstractMatrix{FloatType}, Y::AbstractMatrix{FloatType}, stds::AbstractMatrix{FloatType}; num_training_its::Int=10, rng::Union{Random.MersenneTwister,Nothing}=nothing)
    nd_X = ndims(X)
    nd_Y = ndims(Y)
    nd_std = ndims(stds)
    @assert size(X, nd_X) == size(Y, nd_Y) "Assertion failed: size(X, nd_X) == size(Y, nd_Y). It was ($(size(X, nd_X)) == $(size(Y, nd_Y)))"
    ...
    for it in 1:num_training_its
        for i in order
            ...
            backward_pass(fg, selectdim(Y, nd_Y, i), selectdim(stds, nd_std, i), i)
        end
...
###
mutable struct Trainer{M_G<:AbstractMatrix{Gaussian1d},A_G<:AbstractArray{Gaussian1d}}
    fg::FactorGraph
...
function Trainer(fg::FactorGraph, num_batches::Int)
    # Store messages to weights from batches
    Ws = []
    biases = []
    weight_layers = Vector{WeightFactor}()

    # Why do I need to know the number of batches beforehand? Because I need to know the size of the last dimension of the weight marginals
    ...
    return Trainer(fg, [Ws...], [biases...], weight_layers, Random.MersenneTwister(42))
end
...
function train_batch_new(trainer::Trainer, b::Int, X_b::AbstractMatrix{FloatType}, Y_b::AbstractMatrix{FloatType}, stds_b::AbstractMatrix{FloatType}, num_training_its::Int, β_EMA::Float64, taus_observation::Vector{FloatType}, precisions_observation::Vector{FloatType})
    # Set new priors
    for l_i in eachindex(trainer.Ws)
        W, bias, layer = trainer.Ws[l_i], trainer.biases[l_i], trainer.weight_layers[l_i]
        nd = ndims(W)
        ...
        free_if_CUDA!.((prior_W_, prior_bias_))
    end
    ...
    # If using CPU arrays
    train_batch(trainer.fg, X_b, Y_b, stds_b; num_training_its)

    # Store weight results
    for l_i in eachindex(trainer.Ws)
        W, bias, layer = trainer.Ws[l_i], trainer.biases[l_i], trainer.weight_layers[l_i]
        store_likelihood_messages(layer, selectdim(W, ndims(W), b), selectdim(bias, ndims(bias), b); β_EMA, taus_observation, precisions_observation)
    end
end
...
#         for b in optional_progress_bar(Random.shuffle(trainer.rng, 1:trainer.num_batches), silent)
#             train_batch_new(trainer, b, num_training_its, β_EMA)
#         end
#     end
# end
